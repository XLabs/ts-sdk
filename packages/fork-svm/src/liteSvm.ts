//originally from here https://github.com/LiteSVM/litesvm/blob/master/crates/node-litesvm/litesvm/index.ts
//updated to use @solana/kit instead of the old Solana web3.js sdk
...
export class LiteSVM {
  ...
  simulateTransaction(tx: Transaction): FailedTransactionMetadata | SimulatedTransactionInfo {
    const serialized = getTransactionEncoder().encode(tx) as Uint8Array;
    const result = this.isLegacyTransaction(tx)
    ...
    return result instanceof FailedTransactionMetadata
      ? result
      : new SimulatedTransactionInfo(result);
  }
