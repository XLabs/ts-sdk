...
```typescript
import { serialize, deserialize, type Layout, type DeriveType } from "@xlabs-xyz/binary-layout";
...
type Message = DeriveType<typeof messageLayout>;
// => { version: number; payload: Uint8Array }

const encoded = serialize(messageLayout, { version: 1, payload: new Uint8Array([1, 2, 3]) });
const decoded = deserialize(messageLayout, encoded);
```
...
// Lazy fetching - no upfront account cloning
await fork.sendTransaction(tx);
